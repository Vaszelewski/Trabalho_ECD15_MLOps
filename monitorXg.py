...
def check_for_drift(drift_score, drift_by_columns):
    ...
    if drift_score > 0.5:
        ...
        os.system("python defaultXgBoost.py")
    else:
...
def load_new_data():
    df = pd.read_csv("LoanDefaultPred.csv")
...
def simulate_drift(df_examples):
    ...
    new_data["LoanTerm"] = np.random.randint(0, 10, new_data.shape[0])  # Mudamos a duração do tempo de emprestimo aleatoriamente
    new_data["MonthsEmployed"] *= 1.5  # Aumentamos o tempo de emprego
...
def evaluate_model(df, y, new_data):
    if new_data is None:
        ...
        report.save_html("monitoring_report_df_XG.html")
        ...
        report.save_html("monitoring_report_df_new_data_XG.html")
        report_dict = report.as_dict()
        drift_score = report_dict["metrics"][0]["result"]["dataset_drift"]
        print(f"Score de drift: {drift_score}")
        drift_by_columns = report_dict["metrics"][1]["result"].get("drift_by_columns", {})
